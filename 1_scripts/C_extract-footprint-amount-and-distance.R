...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/conventional seismic cells/areadist_",j,".csv"))
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/low impact seismic cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/paved road cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/gravel road cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/unimproved road cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/pipeline cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
transmissionlines<-pipelines[st_is_valid(transmissionlines)==TRUE,]
#https://gis.stackexchange.com/questions/225157/generate-rectangular-fishnet-or-vector-grid-cells-shapefile-in-r
#create a global object with same extent as human footprint layer 
hf2018_bb <- matrix(c(317461.7, 6431509,
                      830294.2, 6431509,
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/transmission line cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
activewells<-pipelines[st_is_valid(activewells)==TRUE,]
#https://gis.stackexchange.com/questions/225157/generate-rectangular-fishnet-or-vector-grid-cells-shapefile-in-r
#create a global object with same extent as human footprint layer 
hf2018_bb <- matrix(c(317461.7, 6431509,
                      830294.2, 6431509,
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/active well cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
...
abandonedwells<-pipelines[st_is_valid(abandonedwells)==TRUE,]
#https://gis.stackexchange.com/questions/225157/generate-rectangular-fishnet-or-vector-grid-cells-shapefile-in-r
#create a global object with same extent as human footprint layer 
hf2018_bb <- matrix(c(317461.7, 6431509,
                      830294.2, 6431509,
...
for (j in 1:nlevels(as.factor(hf2018_grid_10x10$ID))){
  ...
  if (nrow(pointfile)>0){
    ...
    summaries<-areadist(pointfile,polyfile,maxdist)
    summaries<-data.frame(summaries)
    write.csv(summaries, file=paste0("0_data/1_processed/human footprint/abandoned well cells/areadist_",j,".csv"))
    #Combine the separate files.
  }
  else if(nrow(pointfile)==0){
    print(paste0("No points present and no areas of/distances to features calculated within cell ",j))
